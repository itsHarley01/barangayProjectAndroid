...
class PwdApplicationForm : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_pwd_application_form, container, false)
        val submitButton = view.findViewById<Button>(R.id.button_submit_pwd)
...
//            findNavController().navigate(R.id.action_pwdApplicationForm_to_home2, bundle)
//        }
    ...
    private fun uploadFormDataToFirebase(
        ...
        if (validateForm() && frontIdUri != null && backIdUri != null) {
            ...
            val pendingSubmissionsRef = database.getReference("submissions/forms/pwd/pending")
            ...
            val frontIdImageRef = storageRef.child("Forms/pwd/${userId}_front_id.jpg")
            frontIdImageRef.putFile(frontIdUri!!)
                .addOnSuccessListener { frontIdUploadTask ->
                    frontIdImageRef.downloadUrl.addOnSuccessListener { frontIdUri ->
                        ...
                        val backIdImageRef = storageRef.child("Forms/pwd/${userId}_back_id.jpg")
                                ...
                                backIdImageRef.downloadUrl.addOnSuccessListener { backIdUri ->
                                    ...
                                    pendingSubmissionRef.setValue(submissionData)
                                        .addOnSuccessListener {
                                            Log.d(TAG, "Pwd application uploaded successfully to pending submissions")

                                            userSubmissionsRef.child(pendingSubmissionRef.key ?: "").setValue("")
                                                .addOnSuccessListener {
                                                    Log.d(TAG, "Pwd application associated with user's submissions")
                                                    loadingDialog.dismiss()
                                                    Toast.makeText(requireContext(), "Form Submitted", Toast.LENGTH_SHORT).show()
                                                    //findNavController().navigate(R.id.action_pwdApplicationForm_to_home2, bundle)
                                                }
                                                .addOnFailureListener { e ->
                                                    Log.e(TAG, "Error associating Pwd application with user's submissions: ${e.message}")
                                                }
                                        }
                                        .addOnFailureListener { e ->
                                            Log.e(TAG, "Error uploading Pwd application to pending submissions: ${e.message}")
                                        }
                                }
                            }
                            .addOnFailureListener { e ->
    ...
    companion object {
        private const val TAG = "PwdApplication"
    }
